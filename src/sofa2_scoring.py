...
# Map ventilation_mode to numeric values for model training
VENT_MODE_MAP = {
    'none': 0,
    'high_flow': 1
}
...
def calc_resp(pao2, fio2, vent_mode):
    ...
    # If vent_mode is a string, map it to numeric value
    if isinstance(vent_mode, str):
        vent_mode_num = VENT_MODE_MAP.get(vent_mode, 0)
    else:
        vent_mode_num = vent_mode

    # Determine score based on ventilation_mode
    if vent_mode_num == 1:  # high_flow
        if ratio >= 300: return 0
        elif ratio >= 200: return 1
        elif ratio >= 100: return 3
        else: return 4
    else:  # none
        if ratio >= 400: return 0
        elif ratio >= 300: return 1
...
def calc_cardio(map_val, vaso_dose):
    if pd.isna(map_val):
        return 0
    if map_val >= 70 and vaso_dose == 0: return 0
    elif vaso_dose > 0 and vaso_dose <= 0.1: return 2
    elif vaso_dose > 0.1: return 4
    else: return 1
...
def calculate_sofa2(row):
    ...
    renal_score = calc_renal(row['creatinine'], row['RRT'])  # Fixed: PRT -> RRT
    # Other systems temporarily set to 0
    liver_score = 0
    coag_score = 0
    neuro_score = 0
